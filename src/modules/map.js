...
export const panoramaLayer = new TileLayer({
  title: "panorama lantai 0",
  zIndex: 10,
  source: new TileWMS({
    url: "https://geoserver.observer.xyz/geoserver/assetkukar/wms?service=WMS",
    params: {
      LAYERS: "assetkukar:v_panorama_assetkukar_0",
      TILED: true,
    },
...
export const panoramaLayer2 = new TileLayer({
  title: "panorama lantai 1",
  zIndex: 10,
  source: new TileWMS({
    url: "https://geoserver.observer.xyz/geoserver/assetkukar/wms?service=WMS",
    params: {
      LAYERS: "assetkukar:v_panorama_assetkukar_1",
      TILED: true,
    },
    serverType: "geoserver",
    crossOrigin: "anonymous",
...
export function initMap() {
  ...
  if (isLayerActive.panorama2) {
    containerMap.addLayer(panoramaLayer2);
    mapClickOnPanorama();
  }
...
export function togglePanoramaLayer2(event) {
  const isChecked = event.target.checked;
  if (isChecked) {
    containerMap.addLayer(panoramaLayer2);
    mapClickOnPanorama();
  } else {
    containerMap.removeLayer(panoramaLayer2);
    unByKey(listenerKeyPanorama);
  }
}
...
function mapClickOnPanorama() {
  listenerKeyPanorama = containerMap.on("click", async function (evt) {
    ...
    const featurePanorama2 = panoramaLayer2.getSource().getFeatureInfoUrl(evt.coordinate, viewResolution, viewProjection, { INFO_FORMAT: "application/json" });
    if (featurePanorama) {
    ...
    if (featurePanorama2) {
      const response = await axios.get(featurePanorama2);
      const { features, numberReturned } = response.data;
      if (numberReturned !== 0 && features[0].geometry.type === "Point") {
        isLoadingPanorama.value = true; // enable loading when fetching panorama
        const response = await axiosClient.get(`/panoramas/?project=${getProject()}&base_name=${features[0].properties.base_name}`);
        const { results } = response.data;
        featuresPanorama2.value = results[0];
